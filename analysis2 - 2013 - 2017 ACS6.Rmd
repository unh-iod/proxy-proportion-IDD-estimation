...
for (yr in year){
  ...
  if (!exists("fmlydisb")){
    fmlydisb <- haven::read_sas(paste0("../Data2 - Redo to clean it up/",yr,"/fmlydisb/fmlydisb.sas7bdat"))
    names(fmlydisb) <- names(fmlydisb) %>% str_to_upper()
  } else {
    tmp <- haven::read_sas(paste0("../Data2 - Redo to clean it up/",yr,"/fmlydisb/fmlydisb.sas7bdat"))
    names(tmp) <- names(tmp) %>% str_to_upper()
    fmlydisb <- fmlydisb %>%
      bind_rows(tmp)
  }
...
#Confirming varable availability
...
#Get initial population / proportion estimtates
...
```{r}
combined_clean_acs <- personsx_clean %>%
  left_join(fmlydisb, by = c("HHX","FPX","FMX","SRVY_YR"))

unique(combined_clean_acs$P2DFCON)
```



```{r}
overlap <- combined_clean_acs %>%
...
combined_clean_acs <- personsx_clean %>%
  ...
  filter(RECTYPE.y == 63) %>%
  mutate(cog       = ifelse(P2DFCON ==1, 1, 0),                                 # Cognitive difficulty
         self      = ifelse(P2DFDRES == 1,1,0),                                 # ADL
         ind       = ifelse(P2DFERR == 1, 1, 0),                                # IADL
...
#table(is.na(combined_clean_acs$ind))
#table(combined_clean_acs$RECTYPE.y, combined_clean_acs$SRVY_YR)
#table(combined_clean_acs$RECTYPE.x, combined_clean_acs$SRVY_YR)
#table(combined_clean_acs$cog, combined_clean_acs$SRVY_YR)
#table(combined_clean_acs$self, combined_clean_acs$SRVY_YR)
#table(combined_clean_acs$ind, combined_clean_acs$SRVY_YR)
#table(combined_clean_acs$lesshs, combined_clean_acs$SRVY_YR)
```
...
rowfunc <- function(tab,tots,prev){
  data.frame(
    N11 = round(tab[2,2]),
    POP11 = paste0(round(tots[4]/1000)," (",round(tots[4]/1000 - 2*SE(tots)[4]/1000)," - ",round(tots[4]/1000 + 2*SE(tots)[4]/1000),")"),
    PREV11 = paste0(prev[4]*100,"% (",round(prev[4]*100 - 2*SE(prev)[4]*100,2),"% - ",round(prev[4]*100 + 2*SE(prev)[4]*100,2),"%)"),
    N10 = tab[2,1],
    POP10 = paste0(round(tots[3]/1000)," (",round(tots[3]/1000 - 2*SE(tots)[3]/1000)," - ",round(tots[3]/1000 + 2*SE(tots)[3]/1000),")"),
    PREV10 = paste0(prev[3]*100,"% (",round(prev[3]*100 - 2*SE(prev)[3]*100,2),"% - ",round(prev[3]*100 + 2*SE(prev)[3]*100,2),"%)"),
    N01 = tab[1,2],
    POP01 = paste0(round(tots[2]/1000)," (",round(tots[2]/1000 - 2*SE(tots)[2]/1000)," - ",round(tots[2]/1000 + 2*SE(tots)[2]/1000),")"),
    PREV01 = paste0(prev[2]*100,"% (",round(prev[2]*100 - 2*SE(prev)[2]*100,2),"% - ",round(prev[2]*100 + 2*SE(prev)[2]*100,2),"%)"),
    N00 = tab[1,1],
    POP00 = paste0(round(tots[1]/1000)," (",round(tots[1]/1000 - 2*SE(tots)[1]/1000)," - ",round(tots[1]/1000 + 2*SE(tots)[1]/1000),")"),
    PREV00 = paste0(prev[1]*100,"% (",round(prev[1]*100 - 2*SE(prev)[1]*100,2),"% - ",round(prev[1]*100 + 2*SE(prev)[1]*100,2),"%)")
  )[1,]
}

table(combined_clean_acs$id_, combined_clean_acs$cog)
id_cog_pop  <- svytotal(~id_cog, design = svydes_proxy)
id_cog_prev <- svymean(~id_cog, design = svydes_proxy)
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog), round(id_cog_pop,0), round(id_cog_prev,4))
```

id cross selfcare

```{r}
table(combined_clean_acs$id_, combined_clean_acs$self)
id_self_pop  <- svytotal(~id_self, design = svydes_proxy)
id_self_prev <- svymean(~id_self, design = svydes_proxy)
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$self), round(id_self_pop,0), round(id_self_prev,4))
```

id cross indepenedent living...

```{r}
table(combined_clean_acs$id_, combined_clean_acs$ind)
id_ind_pop  <- svytotal(~id_ind, design = svydes_proxy)
id_ind_prev <- svymean(~id_ind, design = svydes_proxy)
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$ind), round(id_ind_pop,0), round(id_ind_prev,4))
```
...
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog.self), round(id_cog.self_pop,0), round(id_cog.self_prev,4))
```

```{r}
id_cog.ind_pop  <- svytotal(~id_cog.ind, design = svydes_proxy)
id_cog.ind_prev <- svymean(~id_cog.ind, design = svydes_proxy)
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog.ind), round(id_cog.ind_pop,0), round(id_cog.ind_prev,4))
```
...
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog.self.ind), round(id_cog.self.ind_pop,0), round(id_cog.self.ind_prev,4))
```
...
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog.self.ind.lesshs), round(id_cog.self.ind.lesshs_pop,0), round(id_cog.self.ind.lesshs_prev,4))
```
...
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog.ind.lesshs), round(id_cog.ind.lesshs_pop,0), round(id_cog.ind.lesshs_prev,4))
```
...
table(combined_clean_acs$id_, combined_clean_acs$cog.self.lesshs) 
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$cog.self.lesshs), round(id_cog.self.lesshs_pop,0), round(id_cog.self.lesshs_prev,4))
```

```{r}
id_ind.lesshs_pop  <- svytotal(~id_ind.lesshs, design = svydes_proxy)
id_ind.lesshs_prev <- svymean(~id_ind.lesshs, design = svydes_proxy)
table(combined_clean_acs$id_, combined_clean_acs$ind.lesshs)
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$ind.lesshs), round(id_ind.lesshs_pop,0), round(id_ind.lesshs_prev,4))
```
...
table(combined_clean_acs$id_, combined_clean_acs$self.ind.lesshs) 
rowfunc(table(combined_clean_acs$id_, combined_clean_acs$self.ind.lesshs), round(id_self.ind.lesshs_pop,0), round(id_self.ind.lesshs_prev,4))
```

## DD Pop and Prevalence 

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog)
dd_cog_pop  <- svytotal(~dd_cog, design = svydes_proxy)
dd_cog_prev <- svymean(~dd_cog, design = svydes_proxy)
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog), round(dd_cog_pop,0), round(dd_cog_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog.self)
dd_cog.self_pop  <- svytotal(~dd_cog.self, design = svydes_proxy)
dd_cog.self_prev <- svymean(~dd_cog.self, design = svydes_proxy)
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog.self), round(dd_cog.self_pop,0), round(dd_cog.self_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog.ind)
dd_cog.ind_pop  <- svytotal(~dd_cog.ind, design = svydes_proxy)
dd_cog.ind_prev <- svymean(~dd_cog.ind, design = svydes_proxy)
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog.ind), round(dd_cog.ind_pop,0), round(dd_cog.ind_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog.self.ind)
...
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog.self.ind), round(dd_cog.self.ind_pop,0), round(dd_cog.self.ind_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog.self.ind.lesshs)
...
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog.self.ind.lesshs), round(dd_cog.self.ind.lesshs_pop,0), round(dd_cog.self.ind.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog.ind.lesshs) 
...
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog.ind.lesshs), round(dd_cog.ind.lesshs_pop,0), round(dd_cog.ind.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$cog.self.lesshs) 
...
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$cog.self.lesshs), round(dd_cog.self.lesshs_pop,0), round(dd_cog.self.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$dd_, combined_clean_acs$ind.lesshs) 
dd_ind.lesshs_pop  <- svytotal(~dd_ind.lesshs, design = svydes_proxy)
dd_ind.lesshs_prev <- svymean(~dd_ind.lesshs, design = svydes_proxy)
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$ind.lesshs), round(dd_ind.lesshs_pop,0), round(dd_ind.lesshs_prev,4))
```


```{r}
table(combined_clean_acs$dd_, combined_clean_acs$self.ind.lesshs) 
...
rowfunc(table(combined_clean_acs$dd_, combined_clean_acs$self.ind.lesshs), round(dd_self.ind.lesshs_pop,0), round(dd_self.ind.lesshs_prev,4))
```

## IDD Pop and Prevalence 

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog)
idd_cog_pop  <- svytotal(~idd_cog, design = svydes_proxy)
idd_cog_prev <- svymean(~idd_cog, design = svydes_proxy)
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog), round(idd_cog_pop,0), round(idd_cog_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog.self)
idd_cog.self_pop  <- svytotal(~idd_cog.self, design = svydes_proxy)
idd_cog.self_prev <- svymean(~idd_cog.self, design = svydes_proxy)
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog.self), round(idd_cog.self_pop,0), round(idd_cog.self_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog.ind)
idd_cog.ind_pop  <- svytotal(~idd_cog.ind, design = svydes_proxy)
idd_cog.ind_prev <- svymean(~idd_cog.ind, design = svydes_proxy)
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog.ind), round(idd_cog.ind_pop,0), round(idd_cog.ind_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog.self.ind)
...
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog.self.ind), round(idd_cog.self.ind_pop,0), round(idd_cog.self.ind_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog.self.ind.lesshs)
...
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog.self.ind.lesshs), round(idd_cog.self.ind.lesshs_pop,0), round(idd_cog.self.ind.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog.ind.lesshs) 
...
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog.ind.lesshs), round(idd_cog.ind.lesshs_pop,0), round(idd_cog.ind.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$cog.self.lesshs) 
...
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$cog.self.lesshs), round(idd_cog.self.lesshs_pop,0), round(idd_cog.self.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$ind.lesshs) 
...
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$ind.lesshs), round(idd_ind.lesshs_pop,0), round(idd_ind.lesshs_prev,4))
```

```{r}
table(combined_clean_acs$idd_, combined_clean_acs$self.ind.lesshs) 
...
rowfunc(table(combined_clean_acs$idd_, combined_clean_acs$self.ind.lesshs), round(idd_self.ind.lesshs_pop,0), round(idd_self.ind.lesshs_prev,4))
```
...
```{r}
id.x.cog <- table(combined_clean_acs$id_, combined_clean_acs$cog) %>% matrix(nrow = 2) 
...
```{r}
id.x.cog.self <- table(combined_clean_acs$id_, combined_clean_acs$cog.self) %>% matrix(nrow = 2) 
...
```{r}
id.x.cog.ind <- table(combined_clean_acs$id_, combined_clean_acs$cog.ind) %>% matrix(nrow = 2) 
...
```{r}
id.x.cog.self.ind <- table(combined_clean_acs$id_, combined_clean_acs$cog.self.ind) %>% matrix(nrow = 2) 
...
```{r}
id.x.cog.self.ind.lesshs <- table(combined_clean_acs$id_, combined_clean_acs$cog.self.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
id.x.cog.ind.lesshs <- table(combined_clean_acs$id_, combined_clean_acs$cog.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
id.x.cog.self.lesshs <- table(combined_clean_acs$id_, combined_clean_acs$cog.self.lesshs) %>% matrix(nrow = 2) 
...
```{r}
id.x.ind.lesshs <- table(combined_clean_acs$id_, combined_clean_acs$ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
id.x.self.ind.lesshs <- table(combined_clean_acs$id_, combined_clean_acs$self.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog <- table(combined_clean_acs$dd_, combined_clean_acs$cog) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog.self <- table(combined_clean_acs$dd_, combined_clean_acs$cog.self) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog.ind <- table(combined_clean_acs$dd_, combined_clean_acs$cog.ind) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog.self.ind <- table(combined_clean_acs$dd_, combined_clean_acs$cog.self.ind) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog.self.ind.lesshs <- table(combined_clean_acs$dd_, combined_clean_acs$cog.self.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
dd.x.ind.lesshs <- table(combined_clean_acs$dd_, combined_clean_acs$ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
dd.x.self.ind.lesshs <- table(combined_clean_acs$dd_, combined_clean_acs$self.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog.ind.lesshs <- table(combined_clean_acs$dd_, combined_clean_acs$cog.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
dd.x.cog.self.lesshs <- table(combined_clean_acs$dd_, combined_clean_acs$cog.self.lesshs) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog <- table(combined_clean_acs$idd_, combined_clean_acs$cog) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog.self <- table(combined_clean_acs$idd_, combined_clean_acs$cog.self) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog.ind <- table(combined_clean_acs$idd_, combined_clean_acs$cog.ind) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog.self.ind <- table(combined_clean_acs$idd_, combined_clean_acs$cog.self.ind) %>% matrix(nrow = 2) 
...
```{r}
idd.x.ind.lesshs <- table(combined_clean_acs$idd_, combined_clean_acs$ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
idd.x.self.ind.lesshs <- table(combined_clean_acs$idd_, combined_clean_acs$self.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog.ind.lesshs <- table(combined_clean_acs$idd_, combined_clean_acs$cog.ind.lesshs) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog.self.lesshs <- table(combined_clean_acs$idd_, combined_clean_acs$cog.self.lesshs) %>% matrix(nrow = 2) 
...
```{r}
idd.x.cog.self.ind.lesshs <- table(combined_clean_acs$idd_, combined_clean_acs$cog.self.ind.lesshs) %>% matrix(nrow = 2) 
...
id_pred_df <- data.frame(
  dat = combined_clean_acs %>% select(id_,cog,ind,lesshs,PSU,STRAT,WTFA_FDB),
  id_mod_pred = predict(id_mod, type = "response", se = TRUE),
  rnrml = rnorm(dim(combined_clean_acs)[1]),
  all1 = 1)
...
eps <- Inf
cutoff <- sum(id_pred_df$dat.id_)/dim(combined_clean_acs)[1] #unweighted prop
prev_cutoff = 1
...
dd_pred_df <- data.frame(
  dat = combined_clean_acs %>% select(dd_,cog,self,lesshs,PSU,STRAT,WTFA_FDB),
...
eps <- Inf
cutoff <- sum(dd_pred_df$dat.dd_)/dim(combined_clean_acs)[1] #unweighted prop
prev_cutoff = 1
...
dd_table_prop <- svyby(~new_pred, by = ~dat.cog + dat.self + dat.lesshs, design = svydes_mod, svymean) %>%
  mutate(LCI = new_pred - qnorm(.975)*se,
         UCI = new_pred + qnorm(0.975)*se)
dd_table_prop

dd_table_pop <- svyby(~new_pred, by = ~dat.cog + dat.self + dat.lesshs, design = svydes_mod, svytotal) %>%
  mutate(LCI = new_pred - qnorm(.975)*se,
         UCI = new_pred + qnorm(0.975)*se)
dd_table_pop


dd_table_pop_total <- svyby(~all1, by = ~dat.cog + dat.self + dat.lesshs, design = svydes_mod, svytotal) %>%
  mutate(LCI = all1 - qnorm(.975)*se,
         UCI = all1 + qnorm(0.975)*se)
dd_table_pop_total
```
...
idd_pred_df <- data.frame(
  dat = combined_clean_acs %>% select(idd_,cog,ind,lesshs,PSU,STRAT,WTFA_FDB),
...
eps <- Inf
cutoff <- sum(idd_pred_df$dat.idd_)/dim(combined_clean_acs)[1] #unweighted prop
prev_cutoff = 1
...
acs <- acs %>%
  ...
  mutate(dat.cog    = ifelse(is.na(DREM) | DREM == 2, 0, 1),
         dat.self   = ifelse(is.na(DDRS) | DDRS == 2, 0, 1),
...
tus2023_ind <-svyby(~all1,~dat.cog + dat.ind + dat.lesshs,design=acs_des,svytotal,na.rm=T) %>%
  mutate(LCI = all1 - qnorm(.975)*se,
         UCI = all1 + qnorm(0.975)*se)

tus2023_self <-svyby(~all1,~dat.cog + dat.self + dat.lesshs,design=acs_des,svytotal,na.rm=T) %>%
  mutate(LCI = all1 - qnorm(.975)*se,
         UCI = all1 + qnorm(0.975)*se)
...
pred_id2023 <- tus2023_ind %>% 
  ...
  mutate(id_pop = all1*new_pred,
         ...
         LCI_id_pop = id_pop - qnorm(0.975)*se.id_pop,
         UCI_id_pop = id_pop + qnorm(0.975)*se.id_pop)

pred_id2023$id_pop %>% sum()
pred_id2023$se.id_pop^2 %>% sum() %>% sqrt()

pred_dd2023 <- tus2023_self %>% 
  inner_join(dd_table_prop, by = c("dat.cog","dat.self","dat.lesshs"))%>%
  mutate(dd_pop = all1*new_pred,
         se.dd_pop = sqrt(se.x^2*se.y*2 + se.x^2*(new_pred)^2 + se.y^2*(all1)^2),
         LCI_dd_pop = dd_pop - qnorm(0.975)*se.dd_pop,
         UCI_dd_pop = dd_pop + qnorm(0.975)*se.dd_pop)

pred_dd2023$dd_pop %>% sum()
pred_dd2023$se.dd_pop^2 %>% sum() %>% sqrt()


pred_idd2023 <- tus2023_ind %>% 
  ...
  mutate(idd_pop = all1*new_pred,
         ...
         LCI_idd_pop = idd_pop - qnorm(0.975)*se.idd_pop,
         UCI_idd_pop = idd_pop + qnorm(0.975)*se.idd_pop)
...
tus2017_ind <-svyby(~all1,~dat.cog + dat.ind + dat.lesshs,design=acs_des,svytotal,na.rm=T) %>%
  mutate(LCI = all1 - qnorm(.975)*se,
         UCI = all1 + qnorm(0.975)*se)

tus2017_self <-svyby(~all1,~dat.cog + dat.self + dat.lesshs,design=acs_des,svytotal,na.rm=T) %>%
  mutate(LCI = all1 - qnorm(.975)*se,
         UCI = all1 + qnorm(0.975)*se)
...
```{r}
pred_id2017 <- tus2017_ind %>% 
  inner_join(id_table_prop, by = c("dat.cog","dat.ind","dat.lesshs"))%>%
  mutate(id_pop = all1*new_pred,
         se.id_pop = sqrt(se.x^2*se.y*2 + se.x^2*(new_pred)^2 + se.y^2*(all1)^2)) %>%
  mutate(LCI = id_pop - qnorm(0.975)*se.id_pop,
