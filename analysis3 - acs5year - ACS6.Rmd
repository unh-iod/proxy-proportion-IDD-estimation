...
acs5year <- acs5year %>%
  ...
  mutate(cog = ifelse(is.na(DIFFREM) | DIFFREM == 1, 0, 1),
         ...
         self = ifelse(is.na(DIFFCARE) | DIFFCARE == 1, 0, 1),
...
tpuma_ind <- svyby(~all1, by = ~STATEFIP + PUMA + cog + ind + lesshs, design = acs5des, FUN = svytotal)
saveRDS(tpuma_ind,"tpuma_ind.RDS")

#tpuma_self <- svyby(~all1, by = ~STATEFIP + PUMA + cog + self + lesshs, design = acs5des, FUN = svytotal)
#saveRDS(tpuma_ind,"tpuma_self.RDS")
...
```{r}
#load("ACS6-pred.RData")
tpuma_ind <- readRDS("tpuma_ind.RDS") %>%
  rename(all1.se = se)
#tpuma_self <- readRDS("tpuma_self.RDS") %>%
#  rename(all1.se = se)
...
state <- inner_join(
  ...
  relocate(STATEFIP,contains("id_pop"),
           #contains("dd_pop"),
           contains("idd_pop"))
...
puma <- inner_join(
  ...
  mutate(id_pop  = all1.x*id_prop,
         id_pop_se = sqrt(se.x^2*id_prop.se^2 + se.x^2*(id_prop)^2 + id_prop.se^2*(all1.x)^2),
         #dd_pop  = all1.x*dd_prop,
         #dd_pop_se = sqrt(se.x^2*dd_prop.se^2 + se.x^2*(dd_prop)^2 + dd_prop.se^2*(all1.x)^2),
         idd_pop = all1.x*idd_prop,
         idd_pop_se = sqrt(se.x^2*idd_prop.se^2 + se.x^2*(idd_prop)^2 + idd_prop.se^2*(all1.x)^2),
         )%>%
  relocate(STATEFIP,PUMA,contains("id_pop"),contains("dd_pop"),contains("idd_pop"))

puma_total <- puma %>%
  select(STATEFIP,PUMA,all1.y,se.y,N) %>%
  distinct()
...
county_final <- county_pops %>%
  ...
  mutate(county_total = all1.y*ratio,
         county_total_se = sqrt(se.y^2*ratio_se^2 + se.y^2*(ratio)^2 + ratio_se^2*(all1.y)^2),
...
state_final <- state_pops %>%
  ...
  mutate(STATEFIP = sprintf("%02d",STATEFIP)),
  by = c("STATEFIP")) %>%
  select(stab,id_pop,id_pop_se,
         #dd_pop,dd_pop_se,
         idd_pop,idd_pop_se,all1.y,all1.se.y) %>%
...
state_final 
  bind_rows(county_final) %>% writexl::write_xlsx("county-appendix-a5-ACS6.xlsx")
```
